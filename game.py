...
while True:
    ...
    if req == "c":
        ...
        for tool_system, system_name in status_messages:
            if tool_system.now_status():
                print(f"{Fore.RESET}{system_name} {Fore.GREEN}iyi. {Fore.BLUE}{tool_system.status}")
            else:
                print(f"{Fore.RESET}{system_name} {Fore.RED}arızalı!")
        ...
        clear_line(7)
        menu_busy = True
del (req,status_messages,repair_dict,tool,clrscr,getchar,main_thread,tool_system,system_name,Thread,
     main,clock,power_minus,clocker,sp,minus_speed,ses_sistemi,vent,kamera,clear_line,character,name,Fore
)
exit()
